...
const formatDate = (date) => {
  ...
  app.get('/', (req, res) => {
    ...
    let query = 'SELECT title, artist, cover_url, COUNT(*) as play_count FROM radio_songs WHERE date BETWEEN ? AND ? GROUP BY title, artist ORDER BY play_count DESC LIMIT ? OFFSET ?';
...
app.get('/song/:title/:artist', async (req, res) => {
    ...
    const songDetailsQuery = 'SELECT * FROM radio_songs WHERE title = ? AND artist = ? ORDER BY date DESC, time DESC LIMIT 1';
    const playCountByHourQuery = 'SELECT SUBSTRING(time, 1, 2) as hour, COUNT(*) as play_count FROM radio_songs WHERE title = ? AND artist = ? GROUP BY hour ORDER BY hour';
    const playCountByMinuteQuery = 'SELECT SUBSTRING(time, 4, 5) as Minute, COUNT(*) as play_count FROM radio_songs WHERE title = ? AND artist = ? GROUP BY Minute ORDER BY Minute';
    ...
    try {
        ...
        const minute = Array.from({ length: 60 }, (_, i) => i.toString().padStart(2, '0'));
        ...
        const playCountThisWeekQuery = 'SELECT COUNT(*) as play_count FROM radio_songs WHERE title = ? AND artist = ? AND date >= ?';
        const playCountLast30DaysQuery = 'SELECT COUNT(*) as play_count FROM radio_songs WHERE title = ? AND artist = ? AND date >= ?';
        const playCountThisYearQuery = 'SELECT COUNT(*) as play_count FROM radio_songs WHERE title = ? AND artist = ? AND date >= ?';

        const playCountThisWeek = await new Promise((resolve, reject) => {
            db.query(playCountThisWeekQuery, [title, artist, startOfWeek], (err, results) => {
                if (err) reject(err);
        ...
        const playCountLast30Days = await new Promise((resolve, reject) => {
            db.query(playCountLast30DaysQuery, [title, artist, thirtyDaysAgo], (err, results) => {
                if (err) reject(err);
                resolve(results[0].play_count);
            });
        });
        ...
        const playCountThisYear = await new Promise((resolve, reject) => {
            db.query(playCountThisYearQuery, [title, artist, startOfYear], (err, results) => {
                if (err) reject(err);
                resolve(results[0].play_count);
            });
        });
        ...
        const firstPlayDate = playCountPerDay.length > 0 ? new Date(playCountPerDay[0].date) : new Date();
        ...
        const allDates = [];
        const allPlayCounts = [];
        for (let d = new Date(firstPlayDate); d <= lastDate; d.setDate(d.getDate() + 1)) {
